...
local firstSpawnPos = { {screenW*0.1, screenH*0.8}, {screenW/15, screenH*0.81}, {screenW/15, screenH*0.81}, {screenW*0.75, screenH*0.78} }
        ...
        if (phase == "began") then
            ...
            if ("right" == name ) then
				...
				elseif phase=="moved" then
					if (event.x >= (screenW/3) or event.y < (screenH*0.9)) then
						leftM = 0
					end
					if ( event.x < screenW/3 or event.x > screenW*0.6666 or event.y < screenH*0.9) then
						rightM=0
					end
		...
		elseif  (event.selfElement == 2 and event.other.objType == "spring") then
			...
			if ( event.phase == "began" and (isReloading==false)) then
				...
				if (levelNum==5 ) then
					...
					composer.gotoScene( "callback","fade", 500 )
				end
				--physics.stop()
			end
    end
...
function scene:destroy( event )
	...
	local function destRest()
	  ...
	  physics.pause()
		...
		objects=nil
		physics.stop();
		timer.performWithDelay(100, function() ball=nil end)
		if pauseBtn then
